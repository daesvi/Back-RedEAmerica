...
public class PostController {
    ...
    @PostMapping("/create")  // Asegúrate de usar una barra al inicio y especificar el método HTTP
    ...
    @GetMapping  // No es necesario especificar un valor, ya que "/blog" ya está en la clase
    public ResponseEntity<ArrayList<PostEntity>> getPostsByUser() {
        ArrayList<PostEntity> posts = postService.getPostsByUser();
        return ResponseEntity.status(HttpStatus.OK).body(posts);
    }

    @DeleteMapping("/delete/{id}")  // Asegúrate de usar una barra al inicio y especificar el método HTTP
    public ResponseEntity<String> deletePostById(@PathVariable Long id) {
        ...
        return ResponseEntity.status(HttpStatus.BAD_REQUEST)
                .body("Hubo un error al eliminar la publicación, verifica los datos");
    }
}
